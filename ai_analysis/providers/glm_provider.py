...
logger = logging.getLogger("ai_analysis.providers.glm")
...
class GLMProvider(BaseProvider):
    ...
    def chat(self, messages: List[ChatMessage]) -> ProviderResponse:
        if not self.config.api_key:
            logger.warning("GLM API key not configured, using mock")
            return self._get_mock_response("GLM chat")
        ...
        try:
            ...
            logger.error(f"Error in GLM chat: {e}")
